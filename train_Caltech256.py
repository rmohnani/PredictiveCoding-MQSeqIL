...
# # Load Data
# def get_data(batch_size=100, data=6):
...
#     trainset = torchvision.datasets.Caltech256(root='./data', train=True, download=True, transform=transform)

#     train_loader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=True)

#     testset = torchvision.datasets.Caltech256(root='./data', train=False, download=True, transform=transform)

#     test_loader = torch.utils.data.DataLoader(testset, batch_size=100, shuffle=False)
#     print(len(train_loader), len(test_loader))
#     return train_loader, test_loader, d_name, num_train

def get_data(batch_size=100, data=6, test_size=0.2, random_state=42):
    d_name = 'Caltech_256'
    ...
    transform = torchvision.transforms.Compose([
        ...
        torchvision.transforms.Grayscale(num_output_channels=3),  # Ensure each image has 3 color channels
        torchvision.transforms.ToTensor()])
    # Load dataset
    dataset = torchvision.datasets.Caltech256(root=data_dir, download=True, transform=transform)


    # Get labels
    labels = dataset.y
    num_classes = len(dataset.categories)
    print(len(dataset), len(labels), len(dataset.categories))

    train_size = int((1 - test_size) * len(dataset))
    test_size = len(dataset) - train_size

    # Split dataset into train and test sets, ensuring stratified sampling
    ...
    return train_loader, test_loader, d_name, train_size
...
def training_run(epochs=50, batch_size=64, num_seeds=1, alpha=[1], model_type=0, beta=100, arch=0, gamma=.05,
    ...
    for l in range(len(alpha)):
        ...
        for m in range(num_seeds):
            # BP-SGD
            if model_type == 0:
                models[-1].append(IL_Flex.IL(type=0, alpha=alpha[l], output_dim=num_classes))

            # BP-Adam
            elif model_type == 1:
                models[-1].append(IL_Flex.IL(type=1, alpha=alpha[l], output_dim=num_classes))

            # SeqIL
            elif model_type == 2:
                models[-1].append(IL_Flex.IL(n_iter=n_iter, gamma=gamma, beta=beta, type=2, alpha=alpha[l], output_dim=num_classes))

            # SeqIL-MQ
            elif model_type == 3:
                models[-1].append(IL_Flex.IL(n_iter=n_iter, gamma=gamma, beta=beta, type=3, alpha=alpha[l], output_dim=num_classes))

            # SeqIL-Adam
            elif model_type == 4:
                models[-1].append(IL_Flex.IL(n_iter=n_iter, gamma=gamma, beta=beta, type=4, alpha=alpha[l], output_dim=num_classes))
    ...
    #################################################
    # Train
    print(f'\nTRAINING Caltech_256 MODEL TYPE {model_type} SIZE SMALL {small} NITERS {n_iter} BATCH SIZE {batch_size}')
    ...
    with open(f'data/Conv_Type{model_type}_data6_small{small}_niters{n_iter}_epochs{epochs}_{batch_size}.data','wb') as filehandle:
